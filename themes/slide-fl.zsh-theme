...
#
# Recommended crontab:
# nice -n 19 aptitude search "~U" | wc -l > $HOME/.aptcount
...
################################################################################
UPDATES=""
( test -e $HOME/.aptcount ) || echo 0 > $HOME/.aptcount
countupdates=$(cat $HOME/.aptcount)
...
if [ -d "$SLIDE_BATTERY" ]; then
    if [ -e $SLIDE_BATTERY/charge_now ]; then
        batcur=$(cat $SLIDE_BATTERY/charge_now)
        batcap=$(cat $SLIDE_BATTERY/charge_full)
    else
        batcur=$(cat $SLIDE_BATTERY/energy_now)
        batcap=$(cat $SLIDE_BATTERY/energy_full)
    fi
    batsta=$(cat $SLIDE_BATTERY/status)
